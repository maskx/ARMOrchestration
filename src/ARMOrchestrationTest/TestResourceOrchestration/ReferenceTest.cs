    ...
    public class ReferenceTest
    {
        ...
        public void ReferenceDependsOn()
        {
            ...
            foreach (var r in rs)
            {
                ...
                {
                    hasResource = true;
                    var resource = TestHelper.DataConverter.Deserialize<Resource>(r.Input);
                    var v = JObject.Parse(resource.Properties)["comment"];
                    Assert.Equal("Succeeded2020-3-11", v.ToString());
                }
            }
